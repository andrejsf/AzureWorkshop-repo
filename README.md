...
This will create a resource group named `AzureWorkshop-rg`, a workspace named `AzureWorkshop-ws`, and a cluster named `cpu-cluster`. Edit `setup-workspace.py` as needed. If you change the names, ensure you change corresponding names in the `.github/workflows` files and in the third step below.

Third, create a service principal for the resource group:

```console
az ad sp create-for-rbac --name "AzureWorkshop-app" \
                         --role contributor \
                         --scopes /subscriptions/$ID/resourceGroups/AzureWorkshop-rg \
                         --sdk-auth
```
